...
const App: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Bulut veritabanına bağlanılamadı. Lütfen internetinizi kontrol edin.');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="max-w-md w-full space-y-8">
          <div className="text-center space-y-4">
             ...
             <div className="space-y-1">
                ...
                <p className="text-slate-500 text-sm font-bold uppercase tracking-[0.3em]">Cloud Control Center</p>
             </div>
          </div>
          ...
          <div className="bg-white rounded-[2.5rem] p-10 shadow-2xl border border-slate-200/50 relative overflow-hidden group">
            ...
            <form onSubmit={handleLogin} className="space-y-8">
              ...
              <button 
                type="submit"
                disabled={isLoading}
                className="group w-full bg-slate-900 hover:bg-blue-600 text-white font-black py-5 rounded-3xl shadow-xl hover:shadow-blue-500/20 active:scale-[0.97] transition-all text-sm tracking-[0.2em] uppercase flex items-center justify-center gap-3 disabled:opacity-50"
              >
                {isLoading ? <i className="fa-solid fa-spinner fa-spin mr-2"></i> : 'SİSTEME GİRİŞ'}
                <i className="fa-solid fa-arrow-right-long transition-transform group-hover:translate-x-1"></i>
              </button>
            </form>

            <div className="mt-12 text-center">
               <p className="text-[10px] text-slate-300 font-black uppercase tracking-widest">DriverEvent Pro v2.0 - Multi-Device Cloud</p>
            </div>
          </div>
        </div>
      </div>
